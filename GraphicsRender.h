...
#ifndef VULKANPROJECT_GRAPHICSRENDER_H
#define VULKANPROJECT_GRAPHICSRENDER_H
...
const std::string MODEL_PATH = "models/bag.obj";
const std::string TEXTURE_PATH = "textures/bag.jpg";
...
private:
    ...
    static void framebufferResizeCallback(GLFWwindow *window, int width, int height) {
        auto app = reinterpret_cast<GraphicsRender *>(glfwGetWindowUserPointer(window));
    ...
    void createInstance() {
        ...
        appInfo.pEngineName = "Light Voxel Engine";
        ...
        for (const auto &shape : shapes) {
            for(int i = 0; i < 4; ++i)
                for (const auto &index : shape.mesh.indices) {
                    ...
                    vertex.pos = {
                            attrib.vertices[3 * index.vertex_index + 0]/5 + (float)i/2,
                            attrib.vertices[3 * index.vertex_index + 1]/5,
                            attrib.vertices[3 * index.vertex_index + 2]/5 + (float)i/5
                    };
    ...
    void update(uint32_t currentImage) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            vkCmdDraw(commandBuffers[currentImage], vertices.size(), 1, 0, 0);
        }*/
...
#endif //VULKANPROJECT_GRAPHICSRENDER_H